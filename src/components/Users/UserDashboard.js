...
const Dashboard = () => {
  ...
  // Display loading
  if (isLoading) {
    return <StatusMessage type={"loading"} message={"Fetching Data"} />;
  }
  // Check for Error
  else if (isError) {
    return (
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg">
            ...
            <div>
              ...
              {data?.user?.subscriptionPlan === "Trial" && (
                <p className="border rounded w-full py-2 px-3 text-gray-700 leading-tight mb-2">
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg col-span-1 md:col-span-2">
            ...
            {data?.user?.payment?.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                ...
                {data?.user?.payment?.map((payment, index) => {
                  return (
